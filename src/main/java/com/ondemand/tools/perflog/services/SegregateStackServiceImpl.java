...
public class SegregateStackServiceImpl implements SegregateStackService {
    ...
    public  Map<CallCategory,ArrayList<SegregatedStack>>  segregatedStack(CallStack stack, Map<CallCategory,ArrayList<SegregatedStack>> classStackMap){

        if(stack.getN().contains(".dwr")){
            log.info("call :{}", stack.getN());

            SegregatedStack dwrStack = new SegregatedStack
                    (stack.getN(), CallCategory.DWR,true,"callId-000922",
                            stack.getI(),stack.getT(),stack.getM());
            ...
            }else segregatedStackList = List.of(dwrStack).stream()
        ...
        if(stack.getN().contains(".service.")){
            ...
            }else segregatedStackList = List.of(segregatedServiceStack).stream()
                    .collect(Collectors.toCollection(ArrayList::new));

            classStackMap.put(segregatedServiceStack.getCallCategory(),
                    (ArrayList<SegregatedStack>) segregatedStackList);
        }

        if(stack.getN().contains("SQL:")){
            log.info("parsing call :{}", stack.getN());
            SegregatedStack segregatedJdbcStack = new SegregatedStack
                    (stack.getN(), CallCategory.JDBC,true,"callId-000926",
                            stack.getI(),stack.getT(),stack.getM());
            ...
            }else segregatedStackList = (ArrayList<SegregatedStack>) List.of(segregatedJdbcStack).stream()
                    .collect(Collectors.toCollection(ArrayList::new));;
        ...
        if(stack.getN().contains(".cachelegacy.impl.")){
            log.info("parsing call :{}", stack.getN());
            SegregatedStack segregatedCacheStack = new SegregatedStack
                    (stack.getN(), CallCategory.CACHE,true,
            ...
            }else segregatedStackList = List.of(segregatedCacheStack).stream()
                    .collect(Collectors.toCollection(ArrayList::new));;

            classStackMap.put(segregatedCacheStack.getCallCategory(),
                    (ArrayList<SegregatedStack>) segregatedStackList);
